...
pub fn retrieve_cut(
    ...
    if let Some(m) = read_guard.get(k) {
        result.insert(k.clone(), m.no_deps());
    } else {
        panic!("key not found: {} in retrieve_cut", k);
    }
...
pub fn create_snapshot(
    ...
    for k in ri {
        let cut = retrieve_cut(read_guard, k);
        res.merge_into(&cut);
    }
    res
}
