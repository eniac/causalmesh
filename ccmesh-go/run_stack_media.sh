...
NIGHTCORE_ROOT=$(realpath $(dirname $0)/../nightcore)
BUILD_TYPE=release
...
$NIGHTCORE_ROOT/bin/$BUILD_TYPE/gateway \
    --func_config_file=$BASE_DIR/func_config_media.json \
...
$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    ...
    --v=1 2>$BASE_DIR/outputs/launcher_compose_review.log &

$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    --func_id=2 --fprocess_mode=go \
    --fprocess_output_dir=$BASE_DIR/outputs \
    --fprocess=$BASE_DIR/main \
    --v=1 2>$BASE_DIR/outputs/launcher_movie_review.log &

$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    --func_id=3 --fprocess_mode=go \
    --fprocess_output_dir=$BASE_DIR/outputs \
    --fprocess=$BASE_DIR/main \
    --v=1 2>$BASE_DIR/outputs/launcher_user_review.log &

$NIGHTCORE_ROOT/bin/$BUILD_TYPE/launcher \
    --func_id=4 --fprocess_mode=go \
    --fprocess_output_dir=$BASE_DIR/outputs \
    --fprocess=$BASE_DIR/main \
    --v=1 2>$BASE_DIR/outputs/launcher_review_storage.log &
wait
